			...
			public void onClick(View v) {
				try {
					...
					if(ET_barCode.getText().toString().equals("")) {
						Toast.makeText(ProductInfoAddActivity.this, "产品条形码输入不能为空!", Toast.LENGTH_LONG).show();
						ET_barCode.setFocusable(true);
						ET_barCode.requestFocus();
						return;
					}
					productInfo.setBarCode(ET_barCode.getText().toString());
					/*验证获取产品名称*/ 
					if(ET_productName.getText().toString().equals("")) {
						Toast.makeText(ProductInfoAddActivity.this, "产品名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_firstBeizhu.getText().toString().equals("")) {
						Toast.makeText(ProductInfoAddActivity.this, "备注1输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_secondBeizhu.getText().toString().equals("")) {
						Toast.makeText(ProductInfoAddActivity.this, "备注2输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_thirdBeizhu.getText().toString().equals("")) {
						Toast.makeText(ProductInfoAddActivity.this, "备注3输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传产品信息信息*/
					ProductInfoAddActivity.this.setTitle("正在上传产品信息信息，稍等...");
